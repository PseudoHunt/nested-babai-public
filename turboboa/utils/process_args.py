...
def get_turboboa_arguments(**parser_kwargs):
    ...
    ## Nested-lattice refinement (flag-guarded; 'none' => stock TurboBoA)
    parser.add_argument('--nested_arm', type=str, default='none', choices=['none', 'E3', 'E4', 'A', 'B', 'C', 'D'])
    parser.add_argument('--nested_p', type=float, default=0.01, help='refined fraction target (arm A: per-tensor Fisher top-p; arm B: per-column top-k)')
    parser.add_argument('--nested_lambda', type=float, default=None, help='global threshold on gain/N_l (arms C, D)')
...
def get_turboboa_weight_quant_infos(args):
    ...
    if args.nested_arm != 'none':
        from quantizers.nested import NestedRounder
        assert args.w_bits in (2, 3), "nested path: --w_bits is the coarse grid (2 or 3); fine grid is derived (s_c/2, 2 z_c)"
        turboboa_opts['nested'] = NestedRounder(args.nested_arm, p=args.nested_p, lam=args.nested_lambda, log_percol=args.log_percol)
